...
If using nano, use instead of the last two commands something like 
...
Now let's set up the graphical UI and make the environment comfortable. I keep my dotfiles on Github and simply dump them directly into my home directory. You can use them as well:
    ...
    sudo pArch Linux is known for its great wiki, but for a first timer it can be a bit overwhelming with all those options. Having a choice is good as long as that choice is well educated. But what if I don't have any preferences? For that case a simple walkthrough might be in order, like in an RPG-game, you know.

I'd post this to my blog, but I don't have any :/

## The goal

I treat my Linux setup as a project. And like every project it should have a well defined target. I want a fairly minimal but fully functional setup for a laptop with a universal docking solution. As a secondary goal, I want to make an extensive use of the newest Linux developments like systemd and Wayland. For the hardware, I sold my 3 y.o. macbook and for half of the money bought an equally old Thinkpad T480 with a Thunderbolt dock. This walkthrough should work for most of (is not all) recent Thinkpad generations and series. Probably other business laptops too.
...
Now let's set up the graphical UI and make the environment comfortable. I keep my dotfiles on Github and simply dump them directly into my home directory. You can use them as well:
    ...
    cd ~
    git init
    git remote add origin https://github.com/asustavov/dotfiles.git
    git fetch
    git checkout -f master

Now you'll probably want to remove the .git-dir thus de-initializing your home dir as a git repository. You also can just clone the repository somewhere else and copy the files to your home. 
...
Last but not least, install Firefox:

    sudo pacman -S firefox

And... that's it. Now you can reboot, after logging in the (slightly riced) Sway WM in all its glory will automatically load to your desktop. Now feel free to explore the dotfiles, change whatever settings you want to change (keyboard settings most likely, since my Sway is set up to dual German/Russian layout) and call this system your own.acman -S git
    cd ~
    git init
    git remote add origin https://github.com/asustavov/dotfiles.git
    git fetch
